-- Create demo data for Klarsynt organization for onboarding tour
...
DECLARE
  ...
  v_entry_id_5 UUID;
BEGIN
  -- Get Klarsynt organization ID
  SELECT id INTO v_org_id FROM organizations WHERE name = 'Klarsynt' LIMIT 1;
  
  IF v_org_id IS NULL THEN
    RAISE NOTICE 'Klarsynt organization not found, skipping demo data creation';
    RETURN;
  END IF;

  -- Get a form and store for Klarsynt
  ...
  RAISE NOTICE 'Successfully created 5 demo anamnesis entries for Klarsynt organization';
