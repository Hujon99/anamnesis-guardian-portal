...
export function ModalTabContent({
  ...
  return <div className="h-full flex flex-col overflow-hidden">
      ...
      {hasAnswersTab || hasPatientInfoTab ? <Tabs defaultValue={hasAnswersTab ? "answers" : "patient-info"} className="flex flex-col h-full">
          
          
          <div className="flex-grow overflow-hidden">
            {hasAnswersTab && <TabsContent value="answers" className="h-full m-0 border-0 p-0 flex flex-col">
                <OptimizedAnswersView answers={answers} hasAnswers={hasAnswers} status={status} entryId={entry.id} aiSummary={entry.ai_summary} onSaveSummary={onSaveAiSummary} formattedRawData={formattedRawData} setFormattedRawData={setFormattedRawData} saveFormattedRawData={saveFormattedRawData} isPending={isPending} />
              </TabsContent>}
            
            {hasPatientInfoTab && <TabsContent value="patient-info" className="h-full m-0 border-0 p-0 flex flex-col">
                <PatientInfo patientIdentifier={patientIdentifier} isEditing={isEditing} toggleEditing={toggleEditing} setPatientIdentifier={setPatientIdentifier} savePatientIdentifier={savePatientIdentifier} status={status} />
              </TabsContent>}
          </div>
        </Tabs> :
