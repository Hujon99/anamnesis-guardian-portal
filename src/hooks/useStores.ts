...
export function useStores() {
  ...
  const fetchStoresWithRetry = useCallback(async (retryCount = 0) => {
    ...
    try {
      ...
      try {
        ...
        console.log(`useStores: Saved ${data.length} stores to local cache`);
      } catch (cacheError) {
  ...
  useEffect(() => {
    if (stores && stores.length > 0 && !loadingFromCacheRef.current) {
      const newStoreMap = new Map<string, string>();
  ...
  const getStoreName = useCallback((storeId: string | null | undefined): string | null => {
    ...
    if (!name && stores && stores.length > 0) {
      const storeFromArray = stores.find(s => s.id === storeId);
      if (storeFromArray) {
    ...
    try {
      ...
      if (result.isSuccess) {
        toast({
          ...
          description: `${result.data?.length || 0} butiker har hämtats framgångsrikt`,
        });
      }
  ...
  }, [organization?.id, queryClient, refetch, refreshClient]);
