    ...
    return status !== "draft" && <div className="text-center p-4 border border-dashed rounded-md flex-1 min-h-[200px] flex items-center justify-center">
          <div>
            ...
            <p className="text-muted-foreground">
              {status === "sent" ? "Väntar på att patienten ska fylla i anamnesen" : "Ingen information från patienten"}
            </p>
          </div>
        </div>;
  }
  ...
  return <div className="flex flex-col h-full overflow-hidden">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2 items-center">
          {saveIndicator === "unsaved" && <span className="text-amber-500 text-xs">Osparade ändringar</span>}
          {saveIndicator === "saved" && <span className="text-green-500 text-xs flex items-center">
          ...
          {isEditing ? <>
            ...
            setIsEditing(false);
              ...
              <Button variant="default" size="sm" onClick={saveChanges} disabled={isSaving || isPending} className="flex items-center">
              ...
              <Button variant="outline" size="sm" onClick={generateSummary} disabled={isGenerating || !formattedRawData} className="flex items-center">
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="flex flex-col h-full w-full">
          <div className="border-b px-4 pt-2">
            <TabsList className="bg-transparent p-0 h-auto">
              ...
              <TabsTrigger value="summary" className="data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-primary rounded-none" disabled={!summary || summary.trim().length === 0}>
                AI-sammanfattning
              </TabsTrigger>
            </TabsList>
          </div>
          ...
          <div className="flex-grow flex flex-col overflow-hidden">
            <TabsContent value="raw" className="flex-grow m-0 h-full border-0 p-0 overflow-auto">
              {isEditing ? <Textarea value={formattedRawData} onChange={e => {
              updateFormattedRawData(e.target.value);
              setSaveIndicator("unsaved");
            }} className="min-h-[400px] font-mono text-sm h-full w-full resize-none border-0 p-4" placeholder="Redigera svaren eller lägg till anteckningar..." /> : <ScrollArea className="h-full w-full">
                  <div className="p-4">
            ...
            <TabsContent value="summary" className="flex-grow m-0 h-full border-0 p-0 overflow-auto">
                ...
                <div className="p-4">
                  <div className="bg-muted/40 p-4 rounded-md border border-primary/10">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <Button variant="outline" size="sm" onClick={copySummaryToClipboard} className="flex items-center" disabled={!summary || summary.trim().length === 0}>
                        {isCopied ? <>
                    ...
                    <div className="prose prose-sm max-w-none whitespace-pre-wrap">
                      {summary && summary.trim().length > 0 ? summary : "Ingen AI-sammanfattning tillgänglig"}
                    </div>
                  </div>
                </div>
              </ScrollArea>
